...
VERBATIM** (SPEC.md §2.4's flow rule; kogaki#150). You compose the runtime's
*inputs* — the claims, the subdivisions — and relay its *output* as-is:
...
silently vanished at once — the rule binds here because this is the layer
where it was broken.
...
3. **Navigate — screen 2, the co-tag step** —
   ...
   **This is where a tag selection lands, not a second `view --tag`.** A flat
   ...
   - **SubGroups are REQUIRED — the subdivision judgment is not optional**
     (owner ruling 2026-08-07, kogaki#168; SPEC §6.2, §8.1). `--subdivisions`
     is passed on **every** co-tag run: a run that serves no SubGroups on the
     screen and includes none in its Full Reports is a **contract violation
     and a FAILED run**, and any dogfood verdict taken on it is a verdict on
     a failed specimen. **"Required" governs the JUDGMENT, not the outcome** —
     you may not skip the judgment; it does not follow that every group
     subdivides.
   - **SubGroups land where §8's conditions bind**: the CONJUNCTIVE leaf
     condition (composes honestly AND tighter than its
     ...
     condition binds, not a threshold. A group whose leaf condition fails
     renders no SubGroups and is conformant; what is refused is never asking.
   - The screen carries **no per-Strand Gloss line and no Journey line**. That
     ...
     This is a **declared divergence** from the WA baseline, ratified by the
     owner 2026-08-07 (kogaki#167, alternative (b)) and carried at SPEC §2.4
     **register entry 4** — not an oversight, and not to be "repaired" by a
     later sitting that notices the baseline promises a gloss and a journey.
   - **The same act generates the Full Reports, eagerly** (§11 decided, v5,
     ...
     **a report set generated without `--subdivisions` is the 2026-08-07 one**
     (kogaki#168 — the subdivision arguments are no longer optional here).
4. **Read in full — the Full Report** — already generated by the co-tag
   step; `report --survey <record> --tag <T> --group <G> [--claims <F>]
