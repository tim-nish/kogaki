...
// the Harness's sentence about its own arithmetic (§4.11). Re-writing it here
...
export function assembleSelection(reviewed, doc) {
  ...
  // shape §4.11's damaged-ledger clause refuses one field over, arriving by a
  // different door.
  ...
  for (const [i, c] of cands.entries()) {
    ...
    if (owed.length !== rendered) {
      return { error: `candidate ${c.candidate_id} carries ${owed.length} declared `
        ...
        + `(§4.11, §6). Either render it here or regrade the field in `
