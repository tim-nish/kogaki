...
if (( N > FLOOR )); then
  echo "FAIL: the fixture pass reported $N case(s) against a declared case_floor of $FLOOR — cases were ADDED and the floor was not advanced in the same act, so the ratchet is $((N - FLOOR)) behind and cannot see a case deleted inside that gap (kogaki#970). Set case_floor to $N for 'draft-runtime' in checks/registry.json, in this commit"
  exit 1
fi
echo "ok: draft runtime fixture pass ran ${N} case(s) clean, exactly at its declared floor of ${FLOOR}"
